...
~~~nginx
## 1. 查找 Nginx 主进程的 PID
[root@web-server01 /apps/nginx]# cat /apps/nginx/logs/nginx.pid 
9834

## 2. 发送 QUIT 信号到主进程，一旦你获取到 Nginx 主进程的 PID，可以使用 kill 命令发送 QUIT 信号
[root@web-server01 /apps/nginx]# kill -QUIT 9976
...
## 3. 确认 Nginx 已经优雅地关闭
[root@web-server01 /apps/nginx]# ps  -ef | grep   nginx
~~~
...
## 1.关闭nginx进程
[root@web-server01 /root]# nginx  -s stop

## 2. 查看nginx模块
[root@web-server01 /root]# nginx  -V
...
## 3.重新编译
[root@web-server01 /usr/local/src/nginx-1.26.1]# ./configure  --prefix=/apps/nginx --user=nginx --group=nginx\
...
[root@web-server01 /usr/local/src/nginx-1.26.1]# nginx

## 5.验证
[root@web-server01 /usr/local/src/nginx-1.26.1]# curl  -I  www.mmban.com
HTTP/1.1 200 OK
Server: mmban-nginx
...
~~~nginx
## 配置文件
[root@web-server01 /apps/nginx/conf.d]# cat mmban.conf
server {
    listen 80;
...
## html文件
[root@web-server01 /apps/nginx/html.d]# tree  .
.
├── images
│   ├── index.html
│   └── logo.jpg
...
## html文件
[root@web-server01 /apps/nginx/html.d/images]# tree  .
.
├── ab.jpg
├── aB.jpg
├── Ab.jpg
...
## 安装工具
#CentOS安装包
root@web-server01 /apps/nginx/html.d/images~ # yum -y install httpd-tools
#Ubuntu安装包
root@web-server01 /apps/nginx/html.d/images~ # apt -y install apache2-utils

#创建用户
#-b 非交互式方式提交密码
root@web-server01 /apps/nginx/html.d/images~ #  htpasswd -cb /apps/nginx/conf/.htpasswd mmban 123456
Adding password for user mmban
root@web-server01 /apps/nginx/html.d/images~ #  htpasswd -b /apps/nginx/conf/.htpasswd acp 123456
Adding password for user acp
root@web-server01 /apps/nginx/html.d/images~ #  tail /apps/nginx/conf/.htpasswd
...
root@web-server01 /apps/nginx/conf.d~ # curl -I www.mmban.com
HTTP/1.1 200 OK
Server: mmban-nginx
...
~~~nginx
root@web-server01 /usr/local/src/nginx-1.26.1~ # ./configure   --help  | grep -i  with-http_stub_status_module  
  --with-http_stub_status_module     enable ngx_http_stub_status_module

root@web-server01 /usr/local/src/nginx-1.26.1~ # nginx  -V
nginx version: nginx/1.26.1
built by gcc 8.5.0 20210514 (Red Hat 8.5.0-10) (GCC) 
built with OpenSSL 1.1.1k  FIPS 25 Mar 2021
TLS SNI support enabled
...
##分析网站当前访问量
root@web-server01 ~ # curl  www.mmban.com/status  2> /dev/null |awk '/Reading/{print $2,$4,$6}'
0 1 0
~~~
...
~~~nginx
## 创建目录
root@web-server01 /apps/nginx~ # mkdir certs


 ## 自签名CA证书
root@web-server01 /apps/nginx/certs~ # openssl req -newkey rsa:4096 -nodes -sha256 -keyout ca.key -x509 -days 3650 -out ca.crt
...
##自制key和csr文件
root@web-server01 /apps/nginx/certs~ # openssl req -newkey rsa:4096 -nodes -sha256 -keyout www.mmban.com.key -out www.mmban.com.csr
Generating a RSA private key
..................................................................................++++
............++++
...
## 签发证书
root@web-server01 /apps/nginx/certs~ # openssl x509 -req -days 3650 -in www.mmban.com.csr -CA ca.crt -CAkey ca.key -CAcreateserial -out www.mmban.com.crt
Signature ok
...
###验证证书内容
root@web-server01 /apps/nginx/certs~ # openssl x509 -in www.mmban.com.crt -noout -text         
Certificate:
    Data:
        Version: 1 (0x0)
        Serial Number:
        ...
        Issuer: C = CN, ST = GZ, L = GZ, O = GF, OU = devops, CN = web-server01
        Validity
...
root@web-server01 /apps/nginx/certs~ # cat www.mmban.com.crt ca.crt > www.mmban.com.pem 
~~~
...
##重启NGINX服务
root@web-server01 /apps/nginx/conf~ # nginx  -t
nginx: the configuration file /apps/nginx/conf/nginx.conf syntax is ok
nginx: configuration file /apps/nginx/conf/nginx.conf test is successful

root@web-server01 /apps/nginx/conf~ # nginx  -s reload
~~~
...
~~~nginx
###制作key和csr文件
root@web-server01 /apps/nginx/certs~ # openssl req -newkey rsa:4096 -nodes -sha256 -keyout m.mmban.com.key -out m.mmban.com.csr
Generating a RSA private key
.....................................................................++++
.......................................................................................++++
...
##签名证书
root@web-server01 /apps/nginx/certs~ # openssl x509 -req -days 3650 -in m.mmban.com.csr -CA ca.crt -CAkey ca.key -CAcreateserial -out m.mmban.com.crt
Signature ok
subject=C = CN, ST = CQ, L = CQ, O = mmban, OU = mmban, CN = m.mmban.com
Getting CA Private Key

##合并证书文件
root@web-server01 /apps/nginx/certs~ # cat m.mmban.com.crt ca.crt > m.mmban.com.pem
~~~
...
server {
    if ($http_user_agent ~* "(Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini|Firefox)") {
        return 302 https://www.mmban.com.cn$request_uri;
    }
    access_log /apps/nginx/logs/mmban/access_json.log access_json;
...
root@web-server01 /apps/nginx/conf.d~ # cat mmbanorg.conf 
server {
  listen 80;
...
root@web-server01 /apps/nginx/html.d~ # cat mmbanorg/daolian.html
<html>
        <head>
        ...
        <body>
                <img src="http://www.mmban.com/LOGO_MMBAN.png" >
                <h1 style="color:red">欢迎您</h1>
                <p><a href=http://www.mmban.org>MMBAN</a>欢迎您</p>
        </body>
</html>
~~~
...
1. 客户端知道要访问的服务器地址，但服务器只知道请求来自某个代理，而不清楚具体的客户端。正向代理隐藏了真实客户端的信息。例如，当无法直接访问国外网站时，我们通过代理服务器访问特定网址。
2. 使用场景
...
http {
    ...
    server {
        ...
        location / {    
            ...
            #proxy_pass指定的uri不带 / 将访问的/web,等于访问后端服务器，http://192.168.1.18:8080/web/index.html。
            #http://nginx/web/index.html ==> http://192.168.1.18:8080/web/index.html
...
~~~nginx
root@web-server01 /apps/nginx/conf.d~ #  ab -n 2000 -c200 http://www.mmban.com/mmban
...
~~~nginx
#主配置文件
root@web-server01 /apps/nginx~ # cat  conf/nginx.conf
http {
..................
 ...
 #子配置文件
root@web-server01 /apps/nginx~ # cat conf.d/mmban.conf
server {
    location / {
...
################ 验证缓存目录结构及文件大小  
root@web-server01 /apps/nginx~ # tree  proxycache/
proxycache/
└── d
    └── 5
        └── 4
...
######################
root@web-server01 /apps/nginx~ # curl -I  http://www.mmban.com/mmban
HTTP/1.1 302 
Server: mmban-nginx
...
root@web-server01 /apps/nginx/conf~ # cat nginx.conf
http {
...
stream {
 upstream redis_server {
   ...
   server 192.168.1.18:6379 max_fails=3 fail_timeout=30s;
...
stream { 
 ...
 server 192.168.1.7:53; 
 server 192.168.1.17:53; 
   } 
   server { 
   listen 53 udp; 
...
1. PHP-FPM(FastCGI Process Manager：FastCGI进程管理器)是一个实现了Fastcgi的程序，并且提供进程管理的功能。进程包括master进程和worker进程。master进程只有一个，负责监听端口，接受来自web server的请求。worker进程一般会有多个，每个进程中会嵌入一个PHP解析器，进行PHP代码的处理。

##### 20.2.1.Nginx与php-fpm在同一台服务器

~~~nginx
#yum安装默认版本php和相关APP依赖的包
root@web-server01 /apps/nginx/conf~ # yum -y install php-fpm php-mysqlnd php-json
root@web-server01 /apps/nginx/conf~ # ps -ef | grep php-fpm
